...
function initNav(){
  ...
  // Cleaned single-version app.js (no duplicates)
  const app = document.getElementById('app');
  const cartCountEl = document.getElementById('cart-count');
  ...
  async function renderCart(){
    ...
    for(const id of Object.keys(cart)){
      ...
      node.innerHTML = `<img src="${prod.image}" alt="${prod.prompt||prod.name}" style="width:80px;height:60px;object-fit:cover;border-radius:6px"/><div style="flex:1"><div>${prod.name}</div><div class="small">€ ${prod.price.toFixed(2)} x ${qty}</div></div><div><button class="btn" data-plus="${id}" aria-label="Incrementar cantidad">+</button> <button class="btn" data-minus="${id}" aria-label="Disminuir cantidad">-</button></div>`;
